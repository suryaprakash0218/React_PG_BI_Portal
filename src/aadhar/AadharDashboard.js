...
import { getMysqlAadharMonthlyCount, getMysqlAadharTotalCount } from 'src/redux/aadhar/AadharActions';

const AadharDashboard = () => {
  const { allTrans, monthlyTrans } = useSelector((state) => state.aadhar);
  const allData = useSelector((state) => state.aadhar);
  ...
  useEffect(() => {
    ...
    dispatch(getMysqlAadharMonthlyCount());
...
export default AadharDashboard;
