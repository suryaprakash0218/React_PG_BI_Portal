...
import { getMysqlMesevaMonthlyCount, getMysqlMesevaTotalCount } from 'src/redux/meseva/MesevaActions';

const MesevaDashboard = () => {
  const { allTrans, monthlyTrans } = useSelector((state) => state.meseva);
  const allData = useSelector((state) => state.meseva);
  ...
  useEffect(() => {
    ...
    dispatch(getMysqlMesevaMonthlyCount());
...
export default MesevaDashboard;
