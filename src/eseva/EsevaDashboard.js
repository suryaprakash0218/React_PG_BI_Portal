...
import { getMysqlEsevaMonthlyCount, getMysqlEsevaTotalCount } from 'src/redux/eseva/EsevaActions';

const EsevaDashboard = () => {
  const { allTrans, monthlyTrans } = useSelector((state) => state.eseva);
  const allData = useSelector((state) => state.eseva);
  ...
  useEffect(() => {
    ...
    dispatch(getMysqlEsevaMonthlyCount());
