...
import { getMysqlStampsMonthlyCount, getMysqlStampsTotalCount } from 'src/redux/stamps/StampsActions';

const StampsDashBoard = () => {
  const { allTrans, monthlyTrans } = useSelector((state) => state.stamps);
  const allData = useSelector((state) => state.stamps);
  ...
  useEffect(() => {
    ...
    dispatch(getMysqlStampsMonthlyCount());
...
export default StampsDashBoard;
